...
class LoginControllerSpec extends ControllerSpecBase {
  ...
  "Login Controller" must {
    ...
    "redirect to are you in the UK page for Individual when non-uk journeys are toggled on" in {
      ...
      status(result) mustBe SEE_OTHER
      redirectLocation(result) mustBe Some(register.individual.routes.IndividualAreYouInUKController.onPageLoad().url)
    }
    ...
    "redirect to are you in the UK page for organisation when non-uk journeys are toggled on" in {
      ...
      status(result) mustBe SEE_OTHER
      redirectLocation(result) mustBe Some(register.routes.CompanyAreYouInUKController.onPageLoad(NormalMode).url)
    }
  }
}
