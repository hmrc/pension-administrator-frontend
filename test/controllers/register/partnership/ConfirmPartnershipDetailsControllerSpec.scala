...
class ConfirmPartnershipDetailsControllerSpec extends ControllerSpecBase {
  ...
  def controller(dataRetrievalAction: DataRetrievalAction = getEmptyData) =
    new ConfirmPartnershipDetailsController(frontendAppConfig, messagesApi, FakeDataCacheConnector, new FakeNavigator(desiredRoute = onwardRoute), FakeAuthAction,
      dataRetrievalAction, new DataRequiredActionImpl, formProvider)

  def viewAsString(form: Form[_] = form) = confirmPartnershipDetails(frontendAppConfig, form, NormalMode)(fakeRequest, messages).toString
  ...
  "ConfirmPartnershipDetails Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Json.obj(ConfirmPartnershipDetailsId.toString -> true)
