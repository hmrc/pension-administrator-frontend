...
class PartnershipPreviousAddressPostCodeLookupControllerSpec extends ControllerSpecBase {
  ...
  private val onwardRoute = controllers.register.administratorPartnership.routes.PartnershipPreviousAddressListController.onPageLoad(NormalMode)
  ...
  def application: Application = new GuiceApplicationBuilder()
    .overrides(
      ...
      bind[AddressLookupConnector].toInstance(fakeAddressLookupConnector),
      bind(classOf[Navigator]).qualifiedWith(classOf[PartnershipV2]).to(fakeNavigator),
      bind[UserAnswersCacheConnector].toInstance(FakeUserAnswersCacheConnector)
    ).build()
