...
import utils.annotations.{Partnership, PartnershipPartnerV2}
...
class ConfirmDeletePartnerControllerSpec extends ControllerSpecBase {
  ...
  def postUrl: Call = controllers.register.administratorPartnership.routes.AddPartnerController.onPageLoad(NormalMode)
  def redirectUrl: Call = routes.ConfirmDeletePartnerController.onSubmit(firstIndex, NormalMode)
  ...
  "redirect to the next page on a POST request" in {
    running(_.overrides(modules(dataRetrieval)++
      Seq[GuiceableModule](bind[Navigator].qualifiedWith(classOf[PartnershipPartnerV2]).toInstance(new FakeNavigator(postUrl)),
        bind[UserAnswersCacheConnector].toInstance(FakeUserAnswersCacheConnector)
      ):_*)) {
      app =>
