...
class PartnershipPreviousAddressListControllerSpec extends ControllerSpecBase {
  ...
  "partnership Previous Address List Controller" must {
    ...
    "redirect to the next page on POST of valid data" in {
      val onwardRoute = routes.PartnershipEmailController.onPageLoad(NormalMode)
      running(_.overrides(
        bind[AuthAction].to(FakeAuthAction),
        bind[AllowAccessActionProvider].to(FakeAllowAccessProvider(config = frontendAppConfig)),
        bind[UserAnswersCacheConnector].toInstance(FakeUserAnswersCacheConnector),
  ...
  private def addressListViewModel(addresses: Seq[TolerantAddress]): AddressListViewModel = {
    AddressListViewModel(
      ...
      addresses,
      Message("companyPartnership.select.contact.previous.address.heading", Message("thePartnership")),
      Message("companyPartnership.select.contact.previous.address.heading", "Test Partnership Name"),
