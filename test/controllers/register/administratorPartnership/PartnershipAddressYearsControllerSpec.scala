...
import controllers.register.administratorPartnership.routes.PartnershipAddressYearsController
...
class PartnershipAddressYearsControllerSpec extends ControllerSpecBase {
  ...
  def application: Application = new GuiceApplicationBuilder()
    .overrides(
      ...
      bind[Navigator].qualifiedWith(classOf[PartnershipV2]).toInstance(FakeNavigator),
      bind[UserAnswersCacheConnector].toInstance(FakeUserAnswersCacheConnector)
    ).build()
