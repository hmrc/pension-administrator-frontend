...
class ContactDetailsControllerSpec extends ControllerSpecBase {
  ...
  "ContactDetails Controller" must {
    ...
    "populate the view correctly on a GET when the question has previously been answered" in {
      val validData = Json.obj(ContactDetailsId.toString -> ContactDetails("test@test.com", "34892374"))
      val getRelevantData = new FakeDataRetrievalAction(Some(validData))

      val result = controller(getRelevantData).onPageLoad(NormalMode)(fakeRequest)

      contentAsString(result) mustBe viewAsString(form.fill(ContactDetails("test@test.com", "34892374")))
    }
