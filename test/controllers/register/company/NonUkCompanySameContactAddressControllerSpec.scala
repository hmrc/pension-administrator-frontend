...
import identifiers.register.company.{BusinessDetailsId, CompanyAddressId, CompanyNameId, CompanyRegisteredAddressId}
...
class NonUkCompanySameContactAddressControllerSpec extends ControllerSpecBase with CSRFRequest {
  ...
  val dataRetrieval = new FakeDataRetrievalAction(Some(Json.obj(
    CompanyRegisteredAddressId.toString -> address.toAddress,
