  ...
  "PsaDetailsService" must {
    ...
    "when variations and deregistration are enabled" when {
      ...
      "return the correct PSA company view model, also verify the correct existing current address ids and flags" in {
        ...
        UserAnswers(LocalFakeUserAnswersCacheConnector.lastUpsert.get).get(IsAdviserCompleteId).value mustBe true
...
object PsaDetailsServiceSpec extends SpecBase with MockitoSugar {
  ...
  def service(cacheConnector: UserAnswersCacheConnector = LocalFakeUserAnswersCacheConnector) = new PsaDetailServiceImpl(
    fs,
    messagesApi,
