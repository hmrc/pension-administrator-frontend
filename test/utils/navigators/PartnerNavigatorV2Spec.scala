...
import controllers.register.administratorPartnership.partners.routes
...
class PartnerNavigatorV2Spec extends SpecBase with MockitoSugar with NavigatorBehaviour {
  ...
  val navigator: Navigator = injector.instanceOf[PartnerNavigatorV2]
  ...
  "PartnerNavigator in NormalMode" must {
    def routes(): TableFor3[Identifier, UserAnswers, Call] = Table(
      ...
      (AddPartnersId, moreThan1PartnerAddPartnerFalse, partnershipTaskListPage),
      ...
      (MoreThanTenPartnersId, emptyAnswers, partnershipTaskListPage)
...
object PartnerNavigatorV2Spec extends OptionValues {
  ...
  private def partnershipTaskListPage = controllers.register.administratorPartnership.routes.PartnershipRegistrationTaskListController.onPageLoad()
  ...
  private def addPartnersPage(mode: Mode) = controllers.register.administratorPartnership.routes.AddPartnerController.onPageLoad(mode)

  private def moreThanTenPartnersPage(mode: Mode) = controllers.register.administratorPartnership.routes.MoreThanTenPartnersController.onPageLoad(mode)
  ...
  private val addPartnersMoreThan10 = UserAnswers(Json.obj(
    "partners" -> createPartners(numberOfPartnersToCreate = "10".toInt)))
