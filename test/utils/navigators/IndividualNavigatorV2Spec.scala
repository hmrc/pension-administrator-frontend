...
class IndividualNavigatorV2Spec extends SpecBase with NavigatorBehaviour {

  import IndividualNavigatorV2Spec.*

  val navigator: Navigator = injector.instanceOf[IndividualNavigatorV2]

  "IndividualNavigatorV2 in NormalMode" must {
  ...
  "IndividualNavigatorV2 in CheckMode" must {
    def routes(): TableFor3[Identifier, UserAnswers, Call] = Table(
      ("Id", "User Answers", "Next Page"),
      (AreYouInUKId, emptyAnswers, sessionExpiredPage),
      (AreYouInUKId, nonUk, nonUkAdministratorPage),
  ...
  "IndividualNavigatorV2 in UpdateMode" must {
    def routes(): TableFor3[Identifier, UserAnswers, Call] = Table(
      ("Id", "User Answers", "Next Page"),
...
object IndividualNavigatorV2Spec extends OptionValues {
  ...
  lazy private val whatYouWillNeedPage = controllers.register.individual.routes.WhatYouWillNeedController.onPageLoad()
