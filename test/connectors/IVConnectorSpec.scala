...
class IVConnectorSpec extends AsyncWordSpec with MustMatchers with WireMockHelper with OptionValues {

  import IVConnectorSpec._

  override protected def portConfigKey: String = "microservice.services.identity-verification.port"

  private implicit val hc: HeaderCarrier = HeaderCarrier()
  private lazy val connector = injector.instanceOf[IVConnector]

  private val url: String = s"/journey/$journeyId"
  ...
  ".getNinoFromIV" must {
    ...
    "return a Nino" when {
      "IV returned successfully with a nino" in {
        ...
        connector.getNinoFromIV(journeyId).map {
          result =>
      ...
      "throw HttpException" when {
        "no results are found for the requested journey Id" in {
          ...
          recoverToSucceededIf[HttpException] {
            connector.getNinoFromIV(journeyId)
          }
        }
      }
    }
