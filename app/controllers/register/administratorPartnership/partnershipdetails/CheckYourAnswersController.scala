  ...
  def onPageLoad(): Action[AnyContent] = (authenticate andThen getData andThen requireData) {
    implicit request =>
      val sections: Seq[Section] = Seq(
        AnswerSection(None,
          HasPAYEId.cya.row(HasPAYEId)(Some(Link(administratorPartnership.routes.HasPartnershipPAYEController.onPageLoad(CheckMode).url)), request.userAnswers) ++
            EnterPAYEId.cya.row(EnterPAYEId)(Some(Link(administratorPartnership.routes.PartnershipEnterPAYEController.onPageLoad(CheckMode).url)), request.userAnswers) ++
            HasVATId.cya.row(HasVATId)(Some(Link(administratorPartnership.routes.HasPartnershipVATController.onPageLoad(CheckMode).url)), request.userAnswers) ++
            EnterVATId.cya.row(EnterVATId)(Some(Link(administratorPartnership.routes.PartnershipEnterVATController.onPageLoad(CheckMode).url)), request.userAnswers)
        )
      )
