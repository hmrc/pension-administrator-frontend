...
class TestFeatureSwitchManagerController @Inject()(
  ...
  private def switches(featureSwitch:String,
    ...
    "pension-administrator-frontend" -> onOrOff(frontendState),
  ...
  def toggleOn(featureSwitch: String): Action[AnyContent] = Action.async {
    implicit request =>
      ...
      if (frontEndToggledOn) {
        Logger.debug(s"[Pension-Administrator-frontend][ToggleOnSuccess] - $featureSwitch")
      } else {
        Logger.debug(s"[Pension-Administrator-frontend][ToggleOnFailed] - $featureSwitch")
      }
      for {
  ...
  def toggleOff(featureSwitch: String): Action[AnyContent] = Action.async {
    implicit request =>
      ...
      if (frontEndToggledOff) {
        Logger.debug(s"[Pension-Administrator-frontend][ToggleOffSuccess] - $featureSwitch")
      } else {
        Logger.debug(s"[Pension-Administrator-frontend][ToggleOffFailed] - $featureSwitch")
      }
      for {
  ...
  def reset(featureSwitch: String): Action[AnyContent] = Action.async {
    implicit request =>
      ...
      Logger.debug(s"[Pension-Administrator-frontend][ToggleResetSuccess] - $featureSwitch")
      for {
