...
class CheckYourAnswersHelper(userAnswers: UserAnswers, countryOptions: CountryOptions) {
  ...
  def individualContactAddress: Option[AnswerRow] = {
    userAnswers.get(identifiers.register.individual.IndividualContactAddressId) map { answer =>
      AnswerRow("cya.label.individual.contact.address", addressAnswer(answer), answerIsMessageKey = false, None)
    }
  }
  ...
  def individualSameContactAddress: Option[AnswerRow] = {
    userAnswers.get(identifiers.register.individual.IndividualSameContactAddressId) map { answer =>
      AnswerRow("cya.label.individual.same.contact.address", Seq(if (answer) "site.yes" else "site.no"), answerIsMessageKey = true, Some(
  ...
  def individualDateOfBirth: Option[AnswerRow] = userAnswers.get(identifiers.register.individual.IndividualDateOfBirthId) map { x =>
    AnswerRow("individualDateOfBirth.heading", Seq(s"${DateHelper.formatDate(x)}"), answerIsMessageKey = false,
      Some(Link(controllers.register.individual.routes.IndividualDateOfBirthController.onPageLoad(CheckMode).url)),
  ...
  def individualEmail: Option[AnswerRow] = userAnswers.get(identifiers.register.individual.IndividualEmailId) map { x =>
    AnswerRow("individual.email.title", Seq(x), answerIsMessageKey = false,
      Some(Link(controllers.register.individual.routes.IndividualEmailController.onPageLoad(CheckMode).url)),
      Some(Message("individualEmail.visuallyHidden.text")))
  }

  def individualPhone: Option[AnswerRow] = userAnswers.get(identifiers.register.individual.IndividualPhoneId) map { x =>
    AnswerRow("individual.phone.title", Seq(x), answerIsMessageKey = false,
      Some(Link(controllers.register.individual.routes.IndividualPhoneController.onPageLoad(CheckMode).url)),
      Some(Message("individualPhone.visuallyHidden.text")))
  }

  def individualPreviousAddress: Option[AnswerRow] = {
    userAnswers.get(IndividualPreviousAddressId) map { x =>
      AnswerRow("individualPreviousAddress.checkYourAnswersLabel", addressAnswer(x), answerIsMessageKey = false,
  ...
  def individualAddressYears(message: String): Option[AnswerRow] = {
    userAnswers.get(identifiers.register.individual.IndividualAddressYearsId) map { x =>
      AnswerRow(message, Seq(s"common.addressYears.$x"), answerIsMessageKey = true,
  ...
  def individualDetails: Option[AnswerRow] = {
    userAnswers.get(IndividualDetailsId) map { x =>
      AnswerRow("individualDetailsCorrect.name", Seq(s"${x.fullName}"), answerIsMessageKey = false, None)
    }
  }

  def individualAddress: Option[AnswerRow] = {
    userAnswers.get(IndividualAddressId) map { x =>
      AnswerRow("individualDetailsCorrect.address", x.lines(countryOptions), answerIsMessageKey = false, None)
    }
  }

  def moreThanTenDirectors: Option[AnswerRow] = userAnswers.get(identifiers.register.company.MoreThanTenDirectorsId) map {
    x => AnswerRow("moreThanTenDirectors.checkYourAnswersLabel", Seq(if (x) "site.yes" else "site.no"), answerIsMessageKey = true,
