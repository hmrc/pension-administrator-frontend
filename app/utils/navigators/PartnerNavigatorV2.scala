...
import controllers.register.administratorPartnership.partners.routes._
import controllers.register.administratorPartnership.routes.{AddPartnerController, MoreThanTenPartnersController, PartnershipRegistrationTaskListController}
...
@Singleton
class PartnerNavigatorV2 @Inject()(config: FrontendAppConfig) extends Navigator {

  override protected def routeMap(ua: UserAnswers): PartialFunction[Identifier, Call] = {
    normalAndUpdateRoutes(ua, NormalMode) orElse {
      case MoreThanTenPartnersId => PartnershipRegistrationTaskListController.onPageLoad()
    }
  }
  ...
  private def addPartnerRoutes(answers: UserAnswers, mode: Mode): Call = {
    answers.get(AddPartnersId) match {
      case Some(false) if mode == NormalMode => PartnershipRegistrationTaskListController.onPageLoad()
