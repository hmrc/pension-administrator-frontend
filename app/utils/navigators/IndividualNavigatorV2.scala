...
@Singleton
class IndividualNavigatorV2 @Inject()(countryOptions: CountryOptions) extends Navigator {
  ...
  override def routeMap(ua: UserAnswers): PartialFunction[Identifier, Call] = {
    ...
    case WhatYouWillNeedId => IndividualDetailsCorrectController.onPageLoad(NormalMode)
  ...
  def countryOfRegistrationRoutes(answers: UserAnswers): Call = {
    answers.get(AreYouInUKId) match {
      ...
      case Some(true) => WhatYouWillNeedController.onPageLoad()
      case _ => controllers.routes.SessionExpiredController.onPageLoad
    }
  }
