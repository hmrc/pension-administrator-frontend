...
class AddressLookupConnectorImpl @Inject()(httpV2Client: HttpClientV2, config: FrontendAppConfig)
  ...
  override def addressLookupByPostCode(postcode: String)
    ...
    httpV2Client
      ...
      .setHeader(Seq(("X-Hmrc-Origin", "PODS")) *)
      ...
      .flatMap {
        case response if response.status.equals(OK) =>
          Future.successful(
            response
              .json
              .as[Seq[TolerantAddress]](TolerantAddress.postCodeLookupReads)
              .filterNot(
                a =>
                  a.addressLine1.isEmpty && a.addressLine2.isEmpty && a.addressLine3.isEmpty && a.addressLine4.isEmpty
              )
          )
        case response =>
          Future.failed(new HttpException(s"Address Lookup failed with status ${response.status} Response body: ${response.body}", response.status))
      }.recoverWith {
        case t: Throwable =>
