...
class CompanyPreviousAddressFormProvider @Inject() extends Mappings {
   ...
   def apply(): Form[CompanyPreviousAddress] = Form(
     mapping(
      "field1" -> text("companyPreviousAddress.error.field1.required")
        .verifying(maxLength(35, "companyPreviousAddress.error.field1.length")),
      "field2" -> text("companyPreviousAddress.error.field2.required")
        .verifying(maxLength(35, "companyPreviousAddress.error.field2.length"))
