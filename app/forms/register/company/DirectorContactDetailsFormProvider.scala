...
class DirectorContactDetailsFormProvider @Inject() extends Mappings {
   ...
   def apply(): Form[DirectorContactDetails] = Form(
     mapping(
      "field1" -> text("directorContactDetails.error.field1.required")
        .verifying(maxLength(132, "directorContactDetails.error.field1.length")),
      "field2" -> text("directorContactDetails.error.field2.required")
        .verifying(maxLength(24, "directorContactDetails.error.field2.length"))
