...
awk '/class/ {\
     ...
     print "    case Some(x) => Seq(AnswerRow(\"$className;format="decap"$.checkYourAnswersLabel\", s\"\${x.field1} \${x.field2}\", false, controllers.$routeFile$.routes.$className$Controller.onPageLoad(CheckMode).url))";\
     print "    case _ => Nil";\
     print "  }";\
