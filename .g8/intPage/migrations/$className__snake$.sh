...
awk '/class/ {\
     ...
     print "    case Some(x) => Seq(AnswerRow(\"$className;format="decap"$.checkYourAnswersLabel\", s\"\$x\", false, controllers.$routeFile$.routes.$className$Controller.onPageLoad(CheckMode).url))";\
     print "    case _ => Nil";\
     print "  }";\
